...
/* â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
   SEARCH MODAL â€” Spotlight-style command palette
   Opens on âŒ˜K / Ctrl+K or via the topbar's search button. Renders
   above all other UI as a fixed overlay. Keyboard nav: â†‘â†“ navigate,
   â†µ selects, Esc closes. Search functionality is currently a stub
   â€” results filter by simple substring match on labels.
   â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */
const SearchModal = ({ isOpen, onClose, onAction, recent }) => {
  const [query, setQuery] = useState('');
  const [selectedIdx, setSelectedIdx] = useState(0);
  const inputRef = useRef(null);

  /* Build the searchable item set: nav pages + recent + quick actions */
  const allItems = useMemo(() => {
    ...
    (recent || []).slice(0, 5).forEach(r => {
      items.push({
        label: r.label, icon: r.icon || 'overview', path: r.path,
        category: 'Recent',
      });
    });
    items.push(
      { label: 'Generate timetable',       icon: 'bolt',     path: '/view-timetables',    category: 'Actions' },
      { label: 'Add new class',            icon: 'plus',     path: '/classes',            category: 'Actions' },
      { label: 'Add new subject',          icon: 'plus',     path: '/subjects',           category: 'Actions' },
      { label: 'Export reports',           icon: 'report',   path: '/reports',            category: 'Actions' },
      { label: 'Manage academic calendar', icon: 'calendar', path: '/academic-calendar',  category: 'Actions' },
    );
    return items;
  }, [recent]);

  /* Filter by current query (case-insensitive substring) */
  ...
  /* Group filtered items by category, in fixed display order */
  ...
  /* Keyboard navigation â€” â†‘/â†“/Enter/Esc, only while open */
  useEffect(() => {
    if (!isOpen) return;
    const handleKey = (e) => {
      if (e.key === 'Escape') {
  ...
  /* Small kbd-style chip used in the input row + footer hints */
  const kbdStyle = {
  ...
  /* Track running global index across categories so â†‘/â†“ can move
     through the entire list seamlessly regardless of grouping. */
  let globalIdx = 0;
    ...
    >
      <div
        ...
        style={{
          ...
          borderRadius: 16,
          boxShadow:
            '0 24px 80px rgba(20, 20, 40, 0.20), ' +
            '0 4px 16px rgba(20, 20, 40, 0.08), ' +
